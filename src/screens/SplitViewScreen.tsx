    ...
    return (
      <div className="w-full h-full relative overflow-hidden bg-[#EDF1F7]">
        <svg viewBox="0 0 430 300" className="w-full h-full">
          <defs>
            <pattern id="splitMapGrid" width="20" height="20" patternUnits="userSpaceOnUse">
              <path d="M 20 0 L 0 0 0 20" fill="none" stroke="#DCE2EC" strokeWidth="0.6" />
            </pattern>
          </defs>
          ...
          {/* Surrounding blocks */}
          <rect x="8" y="8" width="54" height="26" rx="3" fill="white" stroke="#CAD3E2" strokeWidth="1" />
          <rect x="102" y="8" width="148" height="26" rx="3" fill="white" stroke="#CAD3E2" strokeWidth="1" />
          <rect x="292" y="8" width="130" height="26" rx="3" fill="white" stroke="#CAD3E2" strokeWidth="1" />

          {/* Active Site Zone */}
          <rect x="100" y="70" width="152" height="82" rx="4" fill="#EEF7F4" stroke="#0D9488" strokeWidth="1.5" strokeDasharray="4 3" />
          <rect x="115" y="82" width="122" height="58" rx="2" fill="white" stroke="#0D9488" strokeWidth="1.2" />
          <text x="176" y="112" textAnchor="middle" fontSize="9" fill="#0F766E" fontWeight="bold">CONSTRUCTION SITE</text>
          <text x="176" y="124" textAnchor="middle" fontSize="7" fill="#14B8A6" fontWeight="600">ZONE A · LEVEL 02</text>

          {/* GPS Marker */}
          <circle cx="140" cy="120" r="3.5" fill="#0055ff" stroke="white" strokeWidth="1.2" />
        </svg>

        {/* Floating Pins */}
        <div className="absolute inset-0 pointer-events-none">
          {items.slice(0, 3).map((item, i) => {
            const positions = [{ x: 38, y: 38 }, { x: 55, y: 34 }, { x: 74, y: 65 }];
            const pos = positions[i] || { x: 50, y: 50 };
            const color = item.type === "issue" ? "#EF4444" : item.type === "rfi" ? "#F59E0B" : "#0055ff";
            return (
              <button
                key={item.id}
                type="button"
  ...
  if (type === "3d") {
    return (
      <div
        ...
        style={{
          backgroundImage: "url(https://images.unsplash.com/photo-1486325212027-8081e485255e?w=860&h=500&fit=crop)",
      ...
      >
        ...
        <svg className="absolute inset-0 w-full h-full opacity-20 pointer-events-none">
          <defs>
            <pattern id="splitBimGrid" width="30" height="30" patternUnits="userSpaceOnUse">
    ...
    >
      ...
      {type === "drone" && (
        <svg className="absolute inset-0 w-full h-full pointer-events-none">
          <path d="M 40 80 L 220 90 L 230 180 L 50 190 L 360 220" fill="none" stroke="#0055ff" strokeWidth="1.5" strokeDasharray="4 3" opacity="0.8" />
        </svg>
      )}
      ...
      <div className="absolute inset-0 pointer-events-none">
        {items.slice(0, 3).map((item, i) => {
          const positions = [{ x: 30, y: 38 }, { x: 65, y: 48 }, { x: 45, y: 72 }];
          const pos = positions[i] || { x: 50, y: 50 };
          const color = item.type === "issue" ? "#EF4444" : item.type === "rfi" ? "#F59E0B" : "#0055ff";
          return (
            <button
...
export function SplitViewScreen({
  ...
  const handleDividerPointerDown = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    e.preventDefault();
    ...
    const onMove = (ev: MouseEvent | TouchEvent) => {
      ...
      const fraction = Math.min(Math.max((clientY - rect.top) / rect.height, 0.2), 0.8);
  ...
  return (
    <div className="relative flex flex-col h-full bg-white select-none overflow-hidden">
      ...
      <div ref={containerRef} className="relative flex-1 min-h-0 overflow-hidden bg-slate-900">
        {/* Panel A (Top Panel) */}
        <div
          style={{
            position: "absolute",
        ...
        >
          ...
          <div className="absolute top-3 left-3 z-30">
            ...
            >
              ...
              <span className="leading-none">{PANEL_OPTIONS.find((p) => p.id === panelA)?.label}</span>
              <svg
                width="8"
                height="5"
                viewBox="0 0 10 6"
                fill="none"
                className={`transition-transform duration-200 ${dropdownOpenA ? "rotate-180" : "rotate-0"}`}
              >
                <path d="M1 1L5 5L9 1" stroke="#64748b" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
            ...
            {dropdownOpenA && (
              <>
                <div className="fixed inset-0 z-40" onClick={() => setDropdownOpenA(false)} />
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <div className={isSelected ? "text-[#0055ff]" : "text-slate-500"}>
                              <PanelOptionIcon type={opt.id} />
                            </div>
                            <span>{opt.label}</span>
                          </div>
                          {isSelected && (
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#0055ff" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round">
        ...
        >
          ...
          <div className="absolute top-3 left-3 z-30">
            ...
            >
              ...
              <span className="leading-none">{PANEL_OPTIONS.find((p) => p.id === panelB)?.label}</span>
              <svg
                width="8"
                height="5"
                viewBox="0 0 10 6"
                fill="none"
                className={`transition-transform duration-200 ${dropdownOpenB ? "rotate-180" : "rotate-0"}`}
              >
                <path d="M1 1L5 5L9 1" stroke="#64748b" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
            ...
            {dropdownOpenB && (
              <>
                <div className="fixed inset-0 z-40" onClick={() => setDropdownOpenB(false)} />
                <div className="absolute top-full left-0 mt-1.5 w-40 bg-white/95 backdrop-blur-md rounded-2xl shadow-[0_10px_25px_rgba(0,0,0,0.12)] border border-slate-100 p-1.5 z-50 animate-slide-up">
                  <div className="px-2.5 pt-1 pb-1.5 text-[9.5px] font-bold text-slate-400 uppercase tracking-wider">
                    Panel B View
                  </div>
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100">
              <div>
                <h3 className="text-[15px] font-bold text-slate-900 leading-tight">Markup Layers</h3>
                <p className="text-[11px] text-slate-400 font-medium mt-0.5">Toggle visibility of pins and overlays</p>
              </div>
              <button
                type="button"
                onClick={() => setIsSettingsModalOpen(false)}
              ...
              <div>
                <div className="divide-y divide-slate-100 bg-slate-50 rounded-2xl border border-slate-100 px-3.5">
                  {[
                    {
                      label: "Issues",
                      icon: <span className="w-5 h-5 rounded-md bg-red-100 text-red-500 font-bold text-[10px] flex items-center justify-center">!</span>,
                      value: showIssues,
                    ...
                    {
                      label: "RFIs",
                      icon: <span className="w-5 h-5 rounded-md bg-amber-100 text-amber-600 font-bold text-[10px] flex items-center justify-center">?</span>,
                      value: showRfis,
